...
public class CommentServiceImpl implements CommentService {
    ...
    @Override
    public List<CommentResponse> getPostCommentList(Long postId, Integer page) {
        // 获取一级评论列表
        ...
        // 获取一级评论 comment id
        Set<Long> commentIdSet = parentCommentList.stream().map(CommentDto::getCommentId).collect(Collectors.toSet());
        // 获取一级评论 user id
        Set<Long> userIdSet = parentCommentList.stream().map(CommentDto::getUserId).collect(Collectors.toSet());
        // 根据一级评论 comment id 列表获取最新的一条二级评论及子评论数
        List<CommentDto> childCommentList = commentMapper.selectChildCommentList(commentIdSet);
        ...
        if (!ObjectUtils.isEmpty(childCommentList)) {
            ...
            // 加入二级评论 comment id
            commentIdSet.addAll(childCommentList.stream().map(CommentDto::getCommentId).collect(Collectors.toSet()));
            // 加入二级评论 user id
    ...
    private static List<CommentResponse> merge(List<CommentDto> parentCommentList,
        ...
        return parentCommentList.stream().map(parentComment -> {
            ...
            CommentResponse commentResponse = new CommentResponse()
                    .setParentCommentId(parentComment.getCommentId())
                    .setParentCommentContent(parentComment.getContent())
                    .setParentCommentReleaseTime(DateUtil.format(parentComment.getCreateTime()))
                    .setParentCommentLikedCount(commentLikedCountMap.get(parentComment.getCommentId()))
                    .setParentAuthorInfo(new AuthorDto()
            ...
            if (!ObjectUtils.isEmpty(childComment)) {
                ...
                commentResponse.setChildCommentCount(childComment.getChildCommentCount())
                        .setChildCommentId(childComment.getCommentId())
                        .setChildCommentContent(childComment.getContent())
                        .setChildCommentReleaseTime(DateUtil.format(childComment.getCreateTime()))
                        .setChildAuthorInfo(new AuthorDto()
                        ...
                        .setChildCommentLikedCount(commentLikedCountMap.get(childComment.getCommentId()));
            }
